...
local Run = function()
    ...
    for Property, Value in pairs(Sensor.getTargetDetails("-1,-1,-5")) do
        if ToSend[Property] then
            Table[Property] = Value
        end
    end

    local Sent = http.post("http://127.0.0.1:8000/api/game/reactor", encode(Table))
